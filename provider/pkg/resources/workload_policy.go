...
func (v *VerticalScalingArgs) Annotate(a infer.Annotator) {
	...
	a.Describe(&v.AdjustReqEvenIfNotSet, "Recommend requests even when the workload has no existing requests set. Example: true. Server/web default: true.")
	a.Describe(&v.LimitsRemovalEnabled, "Actively remove limits from workloads (CPU only). Takes precedence over limitsAdjustmentEnabled. Example: true. Web default: true for CPU, false for Memory.")
