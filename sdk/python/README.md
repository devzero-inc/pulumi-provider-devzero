...
| `targetUtilization` | float | Target CPU utilization ratio (0–1). Example: `0.7` |
| `targetMemoryUtilization` | float | Target memory utilization ratio (0–1), tuned independently of CPU. Example: `0.8` |
| `primaryMetric` | string | Primary metric driving HPA (legacy single-metric path, used when `metrics` is empty): `cpu` \| `memory` \| `gpu` \| `network_ingress` \| `network_egress` |
| `maxReplicaChangePercent` | float | Maximum % change in replica count per cycle. Example: `50.0` |
    ...
    hpa_rule=HPARuleConfigArgsArgs(
        ...
        behavior=HPABehaviorArgsArgs(
            scale_down=HPAScalingRulesArgsArgs(
                ...
                select_policy="Min",
                policies=[HPAScalingPolicyArgsArgs(type="Percent", value=10, period_seconds=60)],
            ),
        ),
    ),
))
...
rule, err := resources.NewWorkloadRule(ctx, "my-app-rule", &resources.WorkloadRuleArgs{
    ...
    HpaRule: resources.HPARuleConfigArgsArgs{
        ...
        MaxReplicas:              pulumi.IntPtr(20),
        ScaleDownCooldownSeconds: pulumi.IntPtr(300),
        ...
        Fallback: resources.HPAFallbackArgsArgs{
            ...
            Behavior:         pulumi.String("currentReplicasIfHigher"),
        ...
        Behavior: resources.HPABehaviorArgsArgs{
            ScaleDown: resources.HPAScalingRulesArgsArgs{
                ...
                Policies: resources.HPAScalingPolicyArgsArray{
                    resources.HPAScalingPolicyArgsArgs{
                        Type: pulumi.String("Percent"), Value: pulumi.Int(10), PeriodSeconds: pulumi.Int(60),
                    },
                },
...
| Field | Type | Description |
|---|---|---|
| `replicas` | int | Replica count to use when metrics are unavailable. Example: `2` |
| `behavior` | string | How to apply fallback replicas. One of: `static`, `currentReplicas`, `currentReplicasIfHigher`, `currentReplicasIfLower`. Example: `"currentReplicasIfHigher"` |
